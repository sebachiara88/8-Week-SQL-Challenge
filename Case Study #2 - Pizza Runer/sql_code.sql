...
(SELECT
	A.CUSTOMER_ID,
	AVG(B.DISTANCE) OVER (PARTITION BY A.CUSTOMER_ID) AS AVG_DISTANCE
FROM CUSTOMER_ORDERS A
JOIN RUNNER_ORDERS B
ON A.ORDER_ID = B.ORDER_ID
WHERE B.DISTANCE != 0) AS SUBQUERY
GROUP BY CUSTOMER_ID, AVG_DISTANCE


--What was the total volume of pizzas ordered for each hour of the day?

SELECT
	COUNT(PIZZAS_VOLUME) AS PIZZAS_VOLUME,
	HOUR
FROM(
	SELECT
	ORDER_ID AS PIZZAS_VOLUME,
...
SELECT
	COUNT(PIZZAS_VOLUME_SALES) AS PIZZAS_VOLUME_SALES,
	DAY_OF_WEEK
FROM(
	SELECT
	ORDER_ID AS PIZZAS_VOLUME_SALES,
	CASE 
		WHEN DATEPART(WEEKDAY, ORDER_TIME) = 1 THEN 'MONDAY'
		WHEN DATEPART(WEEKDAY, ORDER_TIME) = 2 THEN 'TUESDAY'
		WHEN DATEPART(WEEKDAY, ORDER_TIME) = 3 THEN 'WEDNESDAY'
		WHEN DATEPART(WEEKDAY, ORDER_TIME) = 4 THEN 'THURSDAY'
		WHEN DATEPART(WEEKDAY, ORDER_TIME) = 5 THEN 'FRIDAY'
		WHEN DATEPART(WEEKDAY, ORDER_TIME) = 6 THEN 'SATURDAY'
		WHEN DATEPART(WEEKDAY, ORDER_TIME) = 7 THEN 'SUNDAY'
		END AS DAY_OF_WEEK
FROM CUSTOMER_ORDERS) AS SUBQUERY
GROUP BY DAY_OF_WEEK
ORDER BY PIZZAS_VOLUME_SALES DESC
